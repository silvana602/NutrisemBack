import { Gender } from 'src/database/enums/gender.enum';

export type PediatricGender = 'male' | 'female';

export type BmiZScoreResult = {
  bmi: number | null;
  zScore: number | null;
...
export function calculateBmi(weightKg: number, heightM: number): number | null {
...
): { zScore: number | null; percentile: number | null } {
  if (
    !Number.isFinite(bmi) ||
    ...
    return { zScore: null, percentile: null };
  }
...
export function getGenderValue(gender: Gender): PediatricGender {
  return gender === Gender.FEMALE ? 'female' : 'male';
}

export function calculatePediatricAnthropometricResult(params: {
  weightKg: number;
  heightM: number;
  birthDate: string | Date;
  gender: Gender | PediatricGender;
  measuredAt?: string | Date;
}): BmiZScoreResult {
  const bmi = calculateBmi(params.weightKg, params.heightM);
  const ageMonths = calculateAgeInMonths(params.birthDate, params.measuredAt);

  if (bmi === null) {
    return { bmi: null, zScore: null, percentile: null, ageMonths };
  }

  const genderValue =
    typeof params.gender === 'string'
      ? params.gender
      : getGenderValue(params.gender);
  ...
  return { bmi, zScore, percentile, ageMonths };
}
